...
const searchSchema = z
  ...
  .refine(
    (schema) =>
      schema.page !== undefined && schema.limit === undefined ? false : true,
    {
  ...
  try {
    ...
    if (seriesCount <= 0) {
      return NextResponse.json(
        { message: "No series found", status: 404 },
        { status: 404 }
      );
    }

    return NextResponse.json(
      { count: seriesCount, series, status: 200 },
      { status: 200 }
    );
  } catch (e) {
    if (e instanceof ZodError) {
      ...
      return NextResponse.json(
        { message: (e as ZodError).message, status: 400 },
        { status: 400 }
      );
    }
  }
